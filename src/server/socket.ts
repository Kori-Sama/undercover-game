...
export default function initSocketServer(httpServer: HTTPServer) {
    ...
    io.on('connection', (socket: Socket) => {
        ...
        // 创建房间
        socket.on('create_room', (roomSettings: Omit<Room, 'roomId' | 'players' | 'host' | 'state' | 'createdAt'>) => {
            // 生成随机房间ID (6位数字)
        ...
        // 开始游戏 (分配角色)
        socket.on('start_game', ({ roomId, playerRoles }: { roomId: string; playerRoles?: Record<string, PlayerRole> }) => {
            const room = rooms.get(roomId);
            ...
            if (playerRoles) {
                ...
                updatedPlayers = updatedPlayers.map(player => {
                    ...
                    return {
                        ...player,
                        role,
                        word: role === "good" ? room.goodWord : role === "evil" ? room.evilWord : undefined
                    };
                });
            } else {
                // 随机分配角色
        ...
        // 结束投票
        socket.on('end_voting', ({ roomId }: { roomId: string }) => {
            const room = rooms.get(roomId);
            ...
            // 清除投票
            const playersWithoutVotes = room.players.map(p => ({ ...p, vote: undefined }));
            ...
            if (maxVotes > alivePlayers / 2 && eliminatedPlayerId) {
                ...
                const updatedPlayers = playersWithoutVotes.map(player => {
                    if (player.id === eliminatedPlayerId) {
                        return { ...player, status: "eliminated" as PlayerStatus };
                    }
                    return player;
                });
                ...
                const updatedRoom = {
                    ...room,
                    players: updatedPlayers,
                    state: (gameEnded ? "ended" : "playing") as GameState,
                    winner: gameEnded ? winner : undefined
                };
                rooms.set(roomId, updatedRoom);
        ...
        // 猜词功能
        socket.on('guess_word', ({ roomId, word }: { roomId: string; word: string }) => {
            const room = rooms.get(roomId);
            ...
            let gameEnded = false;
            let winner: "good" | "evil" | "blank" | undefined = undefined;
...
// 检查游戏是否结束
function checkGameEnd(players: Player[]): { winner: "good" | "evil" | "blank" | undefined, gameEnded: boolean } {
